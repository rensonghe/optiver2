...
#%%
data_2203 = pd.read_csv('Z:/Tick_TQ/ru/ru2005.csv')
...
#%%
data = data_2203[(data_2203.datetime>='2019-11-26 09:00:00')& (data_2203.datetime<='2020-03-18 22:59:59')]
#%%
...
def book_preprocessor(data):
    ...
    df['wap1_shift15'] = df['wap1'].shift(1) - df['wap1'].shift(15)
    df['wap1_shift20'] = df['wap1'].shift(1) - df['wap1'].shift(20)
    df['wap1_shift30'] = df['wap1'].shift(1) - df['wap1'].shift(30)
    ...
    df['wap5_shift10'] = df['wap5'].shift(1) - df['wap5'].shift(10)
    ...
    df['last_price_shift15'] = df['last_price'].shift(1) - df['last_price'].shift(15)
    df['last_price_shift20'] = df['last_price'].shift(1) - df['last_price'].shift(20)
    ...
    df['rolling_bid_size1_shift_mean2'] = df['bid_size1_shift'].rolling(2*rolling).mean()
    df['rolling_bid_size1_shift_mean3'] = df['bid_size1_shift'].rolling(3*rolling).mean()
    ...
    df['rolling_ask_size1_shift_mean2'] = df['ask_size1_shift'].rolling(2*rolling).mean()
    df['rolling_ask_size1_shift_mean3'] = df['ask_size1_shift'].rolling(3*rolling).mean()
    df['rolling_ask_size1_shift_mean5'] = df['ask_size1_shift'].rolling(5*rolling).mean()
    df['rolling_bid_ask_size1_spread_mean2'] = df['bid_ask_size1_spread'].rolling(2*rolling).mean()
    df['rolling_bid_ask_size1_spread_mean3'] = df['bid_ask_size1_spread'].rolling(3*rolling).mean()
    ...
    df['log_return_last_price_shift20'] = np.log(df['last_price'].shift(1) / df['last_price'].shift(20)) * 100
    df['log_return_last_price_shift25'] = np.log(df['last_price'].shift(1) / df['last_price'].shift(25)) * 100
    df['log_return_last_price_shift30'] = np.log(df['last_price'].shift(1) / df['last_price'].shift(30)) * 100
    ...
    df['rolling_last_price_kurt2'] = df['last_price'].rolling(2*rolling).kurt()
    df['rolling_last_price_kurt3'] = df['last_price'].rolling(3*rolling).kurt()
    df['rolling_last_price_kurt5'] = df['last_price'].rolling(5*rolling).kurt()
    ...
    df['rolling_std2'] = df['wap1'].rolling(2*rolling).std()
    df['rolling_std3'] = df['wap1'].rolling(3*rolling).std()
    df['rolling_std5'] = df['wap1'].rolling(5 * rolling).std()
    ...
    df['rolling_skew3_2'] = df['wap3'].rolling(2*rolling).skew()
    df['rolling_skew3_3'] = df['wap3'].rolling(3*rolling).skew()
    df['rolling_skew3_5'] = df['wap3'].rolling(5*rolling).skew()
...
train_data.to_csv('F:/XDQ/ru2005_20191126_20200318_new.csv')