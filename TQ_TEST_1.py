...
api = TqApi(account=sim,backtest=TqBacktest(start_dt=date(2022, 2, 9), end_dt=date(2022, 3, 10)), auth=TqAuth("13575469533", "HZLJDcl123456"),web_gui=True)
...
# gbm = lgb.Booster(model_file='lightGBM_ru.txt')
gbm = lgb.Booster(model_file='lightgbm.txt')
# data = pd.read_csv('2min_feature_columns_ru.csv')
...
def tick_factor_preprocesser(tick_data):
    ...
    # df['wap1'] = tick_data['wap1']
    # df['wap3'] = tick_data['wap3']
    # df['wap5'] = tick_data['wap5']
    # df['wap1_reciprocal'] = reciprocal_transformation(tick_data['wap1'])
    # df['wap1_square_root'] = square_root_translation(tick_data['wap1'])
    # df['wap3_reciprocal'] = reciprocal_transformation(tick_data['wap3'])
    # df['wap3_square_root'] = square_root_translation(tick_data['wap3'])
    # df['wap5_quarticity'] = realized_quarticity(tick_data['wap5'])
    # df['wap5_reciprocal'] = reciprocal_transformation(tick_data['wap5'])
    # df['wap5_square_root'] = square_root_translation(tick_data['wap5'])
    # df['rolling_last_price_reciprocal'] = reciprocal_transformation(tick_data['last_price'])
    # df['rolling_last_price_square_root'] = square_root_translation(tick_data['last_price'])
    # df['rolling_last_price_min'] = tick_data['last_price'].rolling(rolling).min()
    # df['mid_price1'] = (tick_data['ask_price1'] + tick_data['bid_price1']) / 2
    # df['last_price_quantile_25'] = tick_data['last_price'].rolling(rolling).quantile(.25)
    # df['ewm_last_price_mean'] = pd.DataFrame.ewm(tick_data['last_price'], span=rolling).mean()
    # df['rolling_min1'] = tick_data['wap1'].rolling(rolling).min()
    # df['rolling_min3'] = tick_data['wap3'].rolling(rolling).min()
    # df['rolling_quantile3_25'] = tick_data['wap3'].rolling(rolling).quantile(.25)
    # df['ewm_mean3'] = pd.DataFrame.ewm(tick_data['wap3'], span=rolling).mean()
    # df['price_percentile_25'] = tick_data['last_price'].rolling(rolling).quantile(.25)
    # df['last_price'] = tick_data['last_price']
    # df['bid_price1'] = tick_data['bid_price1']
    # df['ask_price1'] = tick_data['ask_price1']
    # df['bid_price2'] = tick_data['bid_price2']
    # df['ask_price2'] = tick_data['ask_price2']
    # df['bid_price3'] = tick_data['bid_price3']
    # df['ask_price3'] = tick_data['ask_price3']
    # df['bid_price4'] = tick_data['bid_price4']
    # df['ask_price4'] = tick_data['ask_price4']
    # df['bid_price5'] = tick_data['bid_price5']
    # df['ask_price5'] = tick_data['ask_price5']
    ...
    if  pos == 1:  #做多
        if position.pos_long == 0 and position.pos_short==0:
            order = api.insert_order(symbol=symbol1, direction="BUY", offset="OPEN", volume=lots,
                                     limit_price=quote1.last_price + slip)#, advanced="FAK")
...
while True:
    ...
    if (time1 >= time4 and time1 < time5):  #交易时间段设置
        ...
        tick1['sec'] = tick1['time'].dt.second
        # tick1 = tick1.reset_index()
        # if tick1['index'] % 120 == 0:
        if tick1['min'].iloc[-1] % 2 == 0 and tick1['min'].iloc[-1] != tick1['min'].iloc[-2]:
            ...
            factor_list = factor_list.reindex(columns=col)
            factor_list = factor_list.iloc[-1]
            final_list = np.array(factor_list).reshape(1,-1)
            # print(factor_list)
            # print(tick1['time'].iloc[-1])
            # print(tick1['last_price'].iloc[-2])
            # print(tick1.last_price.iloc[-1])
            # print('close',quote1.close)
            # print('open', quote1.open)
            pos = main_pos()
            strategy(pos)
