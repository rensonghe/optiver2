...
model_1 = joblib.load('RandomForest_1.pkl')

data = pd.read_csv('test2202_01_04_01_31_new.csv')
data = data.iloc[:,1:79]
col = data.columns
#%%
def factor_generating(data):
    ...
    def factor_preprocesser(data):
        ...
        create_feature_dict = {
            ...
            'W_AB_640': [reciprocal_transformation, square_root_translation],
            'W_A_B_640': [np.mean],
            ...
            'bid_ask_size2_spread': [np.mean, square_root_translation],
    ...
    test_data = test_data.drop(['datetime_'], axis=1)
    test_data = test_data.reindex(columns=col)
    ...
    tick_next = data[-2].reshape(1, -1)
    return tick, tick_next

#%%
def main_pos_1():  #多头交易
    #生成交易信号
    tick_list, tick_list_next = factor_generating(tick1)
    y_pred = model_1.predict(tick_list)
    y_pred_next = model_1.predict(tick_list_next)
    # print('y_pred',y_pred)
    # print('y_pred_next',y_pred_next)
    if y_pred == 1 and y_pred_next == 1:
        pos = 1
    else:
        pos = 0
...
def strategy(pos):
    ...
    if mypos == 0 and pos == 1:  #做多
        openorder = api.insert_order(symbol=symbol1, direction="BUY", offset="OPEN", volume=lots,limit_price=wap_price, advanced="FAK")
        mypos = 1
    if mypos > 0 and pos == 0:  #平多
        closeorder = api.insert_order(symbol=symbol1, direction="SELL", offset="CLOSETODAY", volume=lots,limit_price=wap_price)#, advanced="FAK")
        mypos = 0
