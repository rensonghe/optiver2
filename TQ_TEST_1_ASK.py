...
api = TqApi(account=sim,backtest=TqBacktest(start_dt=date(2022, 1, 7), end_dt=date(2022, 1, 8)), auth=TqAuth("13575469533", "HZLJDcl123456"),web_gui=True)
...
model_1 = joblib.load('RandomForest_1_ASK.pkl')
...
def factor_generating(data):
    ...
    def factor_preprocesser(data):
        ...
        create_feature_dict = {
            ...
            'W_A_B_721': [np.mean, square_root_translation],
            ...
            'W_AB_190': [reciprocal_transformation, square_root_translation],
            'W_A_B_190': [np.mean, square_root_translation],
            'W_AB_280': [reciprocal_transformation, square_root_translation],
            ...
            'W_AB_235': [reciprocal_transformation, square_root_translation],
            'W_A_B_235': [np.mean, square_root_translation],
            'press_buy1': [np.mean, reciprocal_transformation, square_root_translation],
            'ask_size1_shift':[np.mean, square_root_translation],
            'bid_ask_size1_spread':[np.mean, square_root_translation],
            'order_BS1':[np.mean, square_root_translation],
            'vtA':[np.mean, square_root_translation],
            'vtB':[np.mean, square_root_translation],
            'Oiab':[np.mean, square_root_translation],
            'bid_ask_size1_minus': [np.mean, square_root_translation],
            'bid_size1_shift': [square_root_translation],
...
def main_pos_1():  #多头交易
    ...
    y_pred = model_1.predict(tick_list)
    print(y_pred)
    if y_pred == 1:
        pos = 1
    else:
...
def strategy(pos):
    ...
    # print(quote1.ask_price1)
    if mypos == 0 and pos == 1:  #做多
        openorder = api.insert_order(symbol=symbol1, direction="BUY", offset="OPEN", volume=lots,limit_price=quote1.ask_price1, advanced="FAK")
        mypos = 1
    if mypos > 0 and pos == 0:  #平多
        closeorder = api.insert_order(symbol=symbol1, direction="SELL", offset="CLOSETODAY", volume=lots,limit_price=quote1.bid_price1, advanced="FAK")
        mypos = 0
